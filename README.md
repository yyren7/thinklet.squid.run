...
- RTMP(S)/SRTによるライブストリーミング配信
    - Youtube Liveを含む任意のRTMP(S)サーバに配信可能
    - SRTプロトコル対応（弱ネットワーク環境に最適化）
...
| キー                | 値の型     | 必須 | 規定値     | 説明                                                                                             |
|-------------------|---------|----|---------|------------------------------------------------------------------------------------------------|
| `streamUrl`       | String  | ○  |         | RTMP(S)/SRTサーバのURL                                                                             |
...
**NEW!** SquidRunはSRT（Secure Reliable Transport）プロトコルをサポートしています。

#### SRTの利点

- **弱ネットワーク対応**: パケットロスの自動再送により、30%のパケットロスでも正常に配信可能
- **低遅延**: 200ms～8秒の設定可能な遅延
- **暗号化**: AES暗号化による安全な配信
- **産業用途**: 衛星通信、ドローン配信などで広く使用

詳細は [docs/SRT-CONFIGURATION.md](docs/SRT-CONFIGURATION.md) を参照してください。

### テスト用ストリーミングサーバの用意

このアプリを使用したストリーミング機能を動作させるためには、RTMP/SRTサーバーが必要です。
YouTube Liveなどの外部サービスを使用することもできますが、ローカル環境でのテストを行いたい場合は、以下の手順でサーバーを用意することができます。
